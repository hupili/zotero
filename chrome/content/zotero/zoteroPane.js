...
var ZoteroPane = new function() {
    ...
    this.updateItemPaneButtons = function(selectedItems) {
        ...
        document.getElementById('zotero-item-pane-top-buttons-trash').hidden = !this.getCollectionTreeRow().isTrash() || nonDeletedItemsSelected;

        // Feed buttons
        document.getElementById('zotero-item-pane-top-buttons-feed').hidden = !this.getCollectionTreeRow().isFeed()
    };
    ...
    this.buildCollectionContextMenu = async function() {
        ...
        } else if (collectionTreeRow.isHeader()) {} else if (collectionTreeRow.isPublications()) {
            show = [
                'exportFile'
            ];
        }
    ...
    this.showAttachmentNotFoundDialog = function(itemID, noLocate, notOnServer) {
        ...
        var text = Zotero.getString('pane.item.attachments.fileNotFound.text1') + "\n\n" +
            Zotero.getString(
                'pane.item.attachments.fileNotFound.text2' + (notOnServer ? '.notOnServer' : ''), [ZOTERO_CONFIG.CLIENT_NAME, ZOTERO_CONFIG.DOMAIN_NAME]
            );
